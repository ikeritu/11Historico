...
function getCupRoundChaos(roundId: CupFixture["roundId"]): number {
  if (roundId === "round_of_32") return 0.58;
  if (roundId === "round_of_16") return 0.48;
  if (roundId === "quarter_final") return 0.34;
  if (roundId === "semi_final") return 0.22;
  return 0.12;
}
...
function getCupUnderdogBoost(params: {
  ...
  const baseBoost = ratingGap * roundChaos * 0.13;
  const homeBoost = underdogIsHome ? 2.2 : 0.4;

  return clamp(Math.round(baseBoost + homeBoost), 0, 8);
}
...
function applyCupEliteBalance(params: {
  ...
  if (teamIsEliteFavorite) {
    if (roundId === "final") {
      balancedExpected -= 0.2;
    } else if (roundId === "semi_final") {
      balancedExpected -= 0.14;
    } else if (roundId === "quarter_final") {
      balancedExpected -= 0.08;
    }
  }
  ...
  if (teamIsStrongUnderdog) {
    if (roundId === "final") {
      ...
      balancedExpected += 0.16;
    } else if (roundId === "quarter_final") {
      balancedExpected += 0.1;
    }
  }
...
function adjustCupRivalByVenueAndRound(params: {
  ...
  const homeUnderdogBoost = isVerySmall ? 4 : 2;
...
export function simulateNextCupMatch(
  ...
  if (cupResult.userTeamDrew) {
    ...
    const userWinsTieBreaker = seededRandom(tieBreakerSeed) < clamp(0.5 + userAdvantage, 0.35, 0.68);
