...
//#endregion
//#region reports/_tmp_v0_14_3b_balance/runner.ts
...
for (let i = 1; i <= RUNS; i += 1) {
	...
	results.push({
		...
		watchedPositions: {
			...
			Alaves: getPosition(summary, (row) => isTeam(row, "Alav̩s")),
...
for (const result of results) {
	for (const row of result.relegated) for (const watched of watchedTeams) if (norm(row.teamName).includes(norm(watched)) || norm(watched).includes(norm(row.teamName))) relegationCounts[watched] += 1;
...
var alerts = [];
if (villarrealRelegated > 0 || villarrealDanger >= 4) alerts.push("AJUSTAR: Villarreal aparece demasiado abajo o desciende demasiadas veces.");
else alerts.push("OK: Villarreal no exige ajuste con esta muestra.");
