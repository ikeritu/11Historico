...
function testReleaseVersionMetadata(): void {
  ...
  assert(APP_STATUS.includes("palmarés") || APP_STATUS.includes("Palmarés"), "APP_STATUS debe describir el palmarés de carrera.");
...
function testReleaseDocsArePresent(): void {
  ...
  const releaseDocPath = "docs/v0_23_2b5_PALMARES_SUPERCOPA_GAMEOVER_SHOWCASE.md";
  ...
  assert(existsSync(join(ROOT, releaseDocPath)), "Debe existir docs/v0_23_2b5_PALMARES_SUPERCOPA_GAMEOVER_SHOWCASE.md.");
  logOk("README, CHANGELOG y doc de release apuntan a v0.23.2b5");
}
