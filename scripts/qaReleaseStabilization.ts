...
const CURRENT_PUBLIC_VERSION = "v0.23.2b6";
const CURRENT_RELEASE_TAG = "v0.23.2b6_LUCK_WHEEL_LABEL_FIT";
...
function testReleaseVersionMetadata(): void {
  ...
  assert(APP_VERSION_NAME === "Luck Wheel Label Fit", `APP_VERSION_NAME inesperado: ${APP_VERSION_NAME}.`);
  assert(APP_STATUS.includes("Etiquetas compactas") && APP_STATUS.includes("efectos de media"), "APP_STATUS debe describir etiquetas compactas y efectos de media.");
...
function testReleaseDocsArePresent(): void {
  ...
  const releaseDocPath = "docs/v0_23_2b6_LUCK_WHEEL_LABEL_FIT.md";

  assert(changelog.startsWith("# Changelog\n\n## v0.23.2b6"), "CHANGELOG debe empezar por v0.23.2b6.");
  assert(readme.includes(`Versión pública actual: \`${CURRENT_RELEASE_TAG}\`.`), "README debe apuntar a v0.23.2b6_LUCK_WHEEL_LABEL_FIT.");
  assert(existsSync(join(ROOT, releaseDocPath)), "Debe existir docs/v0_23_2b6_LUCK_WHEEL_LABEL_FIT.md.");
  logOk("README, CHANGELOG y doc de release apuntan a v0.23.2b6");
}
