...
const CURRENT_PUBLIC_VERSION = "v0.23.1";
const CURRENT_RELEASE_TAG = "v0.23.1_RELEASE_STABILIZATION";
...
function testReleaseVersionMetadata(): void {
  ...
  assert(APP_VERSION_NAME === "Release stabilization", `APP_VERSION_NAME inesperado: ${APP_VERSION_NAME}.`);
  assert(APP_STATUS.includes("post-ranking global real"), "APP_STATUS debe describir la estabilización post-ranking global real.");
...
function testReleaseDocsArePresent(): void {
  ...
  const releaseDocPath = "docs/v0_23_1_RELEASE_STABILIZATION.md";

  assert(changelog.startsWith("# Changelog\n\n## v0.23.1"), "CHANGELOG debe empezar por v0.23.1.");
  assert(readme.includes(`Versión pública actual: \`${CURRENT_RELEASE_TAG}\`.`), "README debe apuntar a v0.23.1_RELEASE_STABILIZATION.");
  assert(existsSync(join(ROOT, releaseDocPath)), "Debe existir docs/v0_23_1_RELEASE_STABILIZATION.md.");
  logOk("README, CHANGELOG y doc de release apuntan a v0.23.1");
}
