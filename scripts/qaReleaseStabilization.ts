...
const CURRENT_PUBLIC_VERSION = "v0.23.2b2";
const CURRENT_RELEASE_TAG = "v0.23.2b2_LUCK_WHEEL_READABILITY_AND_ARROW_SPEED";
...
function testReleaseVersionMetadata(): void {
  ...
  assert(APP_VERSION_NAME === "Luck Wheel Readability and Arrow Speed", `APP_VERSION_NAME inesperado: ${APP_VERSION_NAME}.`);
  assert(APP_STATUS.includes("Ruleta") || APP_STATUS.includes("ruleta"), "APP_STATUS debe describir la legibilidad y la velocidad progresiva de la ruleta de temporada.");
...
function testReleaseDocsArePresent(): void {
  ...
  const releaseDocPath = "docs/v0_23_2b2_LUCK_WHEEL_READABILITY_AND_ARROW_SPEED.md";

  assert(changelog.startsWith("# Changelog\n\n## v0.23.2b2"), "CHANGELOG debe empezar por v0.23.2b2.");
  assert(readme.includes(`Versión pública actual: \`${CURRENT_RELEASE_TAG}\`.`), "README debe apuntar a v0.23.2b2_LUCK_WHEEL_READABILITY_AND_ARROW_SPEED.");
  assert(existsSync(join(ROOT, releaseDocPath)), "Debe existir docs/v0_23_2b2_LUCK_WHEEL_READABILITY_AND_ARROW_SPEED.md.");
  logOk("README, CHANGELOG y doc de release apuntan a v0.23.2b2");
}
